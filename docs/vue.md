...
使用不可变的数据在维护上更加方便，但也有它的缺点，由于只知道对象改变了，不知道那个地方改变了，所以 React 会对新旧对象 diff，在对象数据很庞大的时候会相当耗时，从而阻塞 ui 线程，界面就会给人卡住的感觉，这也是 Fiber 架构出现的原因。而 Vue 得益于它的依赖收集，使得它可以进行精准更新，从理论上性能是好于 React 的。当然两种框架由于设计理念的不同，走上了不同的道路，没有高低优劣之分，个人认为中小项目更加适合 Vue，大型项目更加适合 React。