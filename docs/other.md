...
## 依赖注入中如何解决循环依赖
假设有两个类 A 和 B，它们相互依赖对方  
### 直接引用变间接引用
可以在 A 中定义一个抽象类 C，C 由外部注入而来，此时 A 就由依赖 B 变为依赖自己文件中定义的 C。而 B 会在外部主动注入 C 的实现
### 分离依赖部分，打破依赖链
A 依赖 B 不一定依赖 B 的全部，此时拆分 B，将其 A 依赖的部分拆分为 B1，剩余的为 B2。则此时的依赖链为：B2 => A => B1，不存在环则不会有循环依赖的问题。
### 先初始化某一个，再注入
A B 相互依赖不一定初始化的时候就用到了，可以先初始化其中一个 A 将其依赖的 B 设置为 null，初始化 A 完成后将其丢给 B 初始化，B 初始化完成后再将其赋值给 A。此方法的优点是不需要调整代码的代码的，大部分依赖注入也是使用的这个方法。但它有很大的局限性，只能处理再初始化中不相互依赖的情况。
